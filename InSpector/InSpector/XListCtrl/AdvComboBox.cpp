...
void CAdvComboBox::LoadString( UINT nStringID )
{
	...
	_tcscpy_s( szTok, nLen+5, (LPCTSTR)strItems );

	token = _tcstok_s( szTok, seps, &nextToken);
	while( token != NULL )
	{
		AddString( token );
...
int CAdvComboBox::GetLBText(int nIndex, LPTSTR lpszText)
{
	...
	_tcscpy_s( lpszText, m_iter->strText.size(), m_iter->strText.c_str() );
...
int CAdvComboBox::GetText(LPTSTR lpszText, size_t size)
{
	if( m_pEdit )
	{	
		CString str;
		m_pEdit->GetWindowText( str );
		_tcscpy_s( lpszText, size, (LPCTSTR)str );
		return str.GetLength();
	}
	else
	{
		_tcscpy_s( lpszText, size, m_strEdit.c_str() );
