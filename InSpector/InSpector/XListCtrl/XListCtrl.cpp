...
BEGIN_MESSAGE_MAP(CXListCtrl, CListCtrl)
	...
	ON_WM_RBUTTONDOWN()
	ON_WM_NCLBUTTONDOWN()
	//ON_MESSAGE(WM_XLISTCTRL_UPDATE, OnListCtrlUpdate)
	//}}AFX_MSG_MAP
...
//
BOOL CXListCtrl::SetItemText(int nItem, int nSubItem, LPCTSTR lpszText, COLORREF crText, COLORREF crBackground)
{
	ASSERT(nItem >= 0);
	ASSERT(nItem < GetItemCount());
	if ((nItem < 0) || nItem >= GetItemCount())
...
/**
* @reamarks
*	리스트 컨트롤에 해당 문자열을 매개변수 정보 스타일로 추가
* @author 김동현
* @date 2016/06/08
* @param int n	리스트 컨트롤 헤더로 지정할 문자열의 수
* @param LISTCTRLHEADER* pListCtrlHeader	리스트 컨트롤 헤더로 지정할 문자열 셋
* @param bool bVScroll	리스트 컨트롤 헤더로의 수직 스크롤바 사용 여부
*/
void CXListCtrl::InitListCtrlHeader(int n, LISTCTRLHEADER* pListCtrlHeader, bool bVScroll)
{
	ASSERT(this);
	if (!this)
		return;
	...
	w = (bVScroll) ? w - ::GetSystemMetrics(SM_CXVSCROLL) -2 /*margine*/ : w;

	//m_colWidth = new int[n];
	...
	for(int i=0; i<n; i++)
	{
		...
		lvcolumn.iSubItem = i;
		lvcolumn.pszText = pListCtrlHeader[i].pszText;
		lvcolumn.fmt = pListCtrlHeader[i].fmt;
		lvcolumn.cx = (i == n-1) ? w - total_cx - 2 : (int)(w * (pListCtrlHeader[i].width / 100.0f));
		//m_colWidth[i] = pListCtrlHeader[i].width;
	...
	// 여기 - m_HeaderCtrl.GetItemCount
	for (int i = 0; i < this->m_HeaderCtrl.GetItemCount(); i++)
	{
		hditem.mask = HDI_IMAGE | HDI_FORMAT;
		// 여기 --> m_HeaderCtrl.GetItem
		this->m_HeaderCtrl.GetItem(i, &hditem);
		...
		// 여기 --> m_HeaderCtrl.SetItem
		this->m_HeaderCtrl.SetItem(i, &hditem);
	}
	...
	for(int i=0; i<n; i++)
	{
		...
		lvcolumn.iSubItem = i;
		// 여기- SetColumn
		this->SetColumn(i, &lvcolumn);
	}
}

//LRESULT CXListCtrl::OnListCtrlUpdate(WPARAM wParam, LPARAM lParam)
//{
//	CString* str = (CString*)lParam;
//	SetItemText((wParam >> 8) & 0xff, wParam & 0xff, *str);
//
//	return 0;
//}