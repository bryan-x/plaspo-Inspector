...
//#define WM_XLISTCTRL_UPDATE (WM_USER + 100)


/**
* @brief 리스트 컨트롤 헤더 초기화 편리를 위한 구조체
* @author 김동현
* @date 2016/06/08
*/
struct LISTCTRLHEADER
{
	TCHAR* pszText;			/**< 헤더로 등록할 문자열 */
	int width;				/**< 리스트 컨트롤에서 차지하는 비율 */
	int fmt;				/**< 헤더 정렬 */
	...
	LISTCTRLHEADER(TCHAR* _pszText, int _width, int _fmt = LVCFMT_CENTER)
	{
		pszText = _pszText;
		width = _width;
		fmt = _fmt;
	...
	//LISTCTRLHEADER(const LISTCTRLHEADER& lcHeader)
	//{
	//	str = lcHeader.str;
	//	width = lcHeader.width;
	//}
};
...
public:
	void InitListCtrlHeader(int n, LISTCTRLHEADER* pListCtrlHeader, bool bVScroll = true);

	//void AutoSetColumnWidth(int cx);
};
