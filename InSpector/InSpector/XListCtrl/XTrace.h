...
#define XTRACE_SHOW_THREAD_ID	FALSE	// TRUE = include thread id in output
#define XTRACE_FILE				FALSE	// TRUE = output to file
#define XTRACE_SOURCECODE		FALSE	// TRUE = show .cpp(.c) file name and code line
...
public:
	...
	void operator() (LPCTSTR lpszFormat, ...)
	{
		...
		{
			if (_sntprintf_s(buf1, BUFFER_SIZE-1, _T("%s"), lpszFormat) < 0)
				buf1[BUFFER_SIZE-1] = _T('\0');
		}
